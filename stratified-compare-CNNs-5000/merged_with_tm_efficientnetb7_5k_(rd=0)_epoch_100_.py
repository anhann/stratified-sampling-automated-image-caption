# -*- coding: utf-8 -*-
"""Merged - With TM - EfficientNetB7 - 5k (rd=0) - epoch 100 .ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1mJnzpvfbxmT2BfaYX90VIFfAZgAf25LW

# THE WORK BELOW REFERS TO 3 SOURCES OF CODE SAMPLE:
...
3. BLEU calculation and photo illustration:
...
This notebook is an end-to-end example. When you run the notebook, it downloads the [MS-COCO](http://cocodataset.org/#home) dataset, preprocesses and caches a subset of images using EfficientNetB7, trains an encoder-decoder model, and generates captions on new images using the trained model.
"""
...
#create 2 list, 1 with all captions (30026), one with image path (each path repeat 5 times) so that the positions match
train_captions = []
img_name_vector = []
...
"""## Preprocess the images using EffNetB7
...
"""## Initialize EffNetB7 and load the pretrained Imagenet weights

"""

pip install efficientnet

image_model = tf.keras.applications.efficientnet.EfficientNetB7(include_top=False,
                                                weights='imagenet')
...
"""## Caching the features extracted from EffNetB7

You will pre-process each image with InceptionV3 and cache the output to disk. Caching the output in RAM would be faster but also memory intensive, requiring 18 \* 18 \* 2560 floats per image. At the time of writing, this exceeds the memory limitations of Colab (currently 12GB of memory).
...
# create dataframe to store image path
import pandas as pd
caption_table=pd.DataFrame(list())
caption_table['image_path']=train_image_paths
caption_table['text']=['']*len(train_image_paths)
caption_table
...
"""Refer from https://www.machinelearningplus.com/nlp/topic-modeling-gensim-python/"""
...
model_lda =  ldamodel.LdaModel(corpus=corpus, num_topics=30, id2word=id2word, random_state=42)
def format_topics_sentences(model_lda, corpus):
    # Init output
...
df_topic_sents_keywords = format_topics_sentences(model_lda=model_lda, corpus=corpus)
...
# Create training and validation sets with topic modeling (for each topic ~80% (4000) for train and 20% for test)
img_name_train_keys =[]
img_name_val_keys = []
for i in range(30):
    group= df_dominant_topic[df_dominant_topic['Dominant_Topic']==i]['image_path']
    image_paths = list(group.values)
    random.Random(4).shuffle(image_paths)
    slice_ind=int(len(image_paths)*0.8031)
    img_name_train_keys.extend(image_paths[:slice_ind])
    img_name_val_keys.extend(image_paths[slice_ind:])

# image and respective caption
img_name_train = []
cap_train = []
...
# Shape of the vector extracted from EffNetB7 is (324, 2560)
...
* In this example, you extract the features from the lower convolutional layer of EffNetB7 giving us a vector of shape (18, 18, 2560).
* You squash that to a shape of (324, 2560).
