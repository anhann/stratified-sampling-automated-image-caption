# -*- coding: utf-8 -*-
"""Without TM - Inception - 5k (rd=0).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1jpjhGv4MR3-kT8KcusQrxR428kwGTh_1
...
# **EVALUATION WITH BLEU SCORE - ONE VS ONE**
"""

# Refer from: https://www.kaggle.com/anunay999/image-captioning-using-vgg16
score1=0
nkeep = 10
pred_good1, pred_bad1, pred1, bleus1 = [], [], [], []
from nltk.translate.bleu_score import sentence_bleu
for i in range(len(img_name_val)):
    image = img_name_val[i]
    reference =[tokenizer.index_word[m] for m in cap_val[i] if m not in [0]][1:-1]
    result, attention_plot = evaluate(image)
    candidate = result[:-1]
    bleu= sentence_bleu([reference], candidate)
    score1 += bleu
    bleus1.append(bleu)
    pred1.append((bleu,image,reference,candidate))
    if bleu > 0.8 and len(pred_good1) < nkeep:
        pred_good1.append((bleu,image,reference,candidate))
    elif bleu < 0.3 and len(pred_bad1) < nkeep:
        pred_bad1.append((bleu,image,reference,candidate))

bleu_score1=score1/len(img_name_val)
print(bleu_score1)

# Refer from: https://www.kaggle.com/anunay999/image-captioning-using-vgg16
from keras.preprocessing.image import load_img
def plot_images1(pred_bad):
...
print("Bad Caption")
plot_images1(pred_bad1)
print("Good Caption")
plot_images1(pred_good1)
...
# Plot BLEU distribution in 2 approaches
from matplotlib.ticker import PercentFormatter
from matplotlib.pyplot import figure

figure(figsize=(7,4))
bins = np.linspace(0, 1, 10)
plt.hist(bleus1, bins, alpha=0.5, label='one-to-one',weights=np.ones(len(bleus1)) / len(bleus1),color = "gray")
plt.hist(bleus2, bins, alpha=0.5, label='one-to-all',weights=np.ones(len(bleus2)) / len(bleus2),color = "deepskyblue")
...
x=870
pred_oto=pred1[x*5+2:x*5+7]
pred_ota=[pred2[x]]
print('One to one')
plot_images1(pred_oto)
print('\nOne to all')
plot_images2(pred_ota)

for i in pred_oto:
  print(sentence_bleu(pred_ota[0][2],i[3]))

pred_ota[0][2]

# Save bleu and prediction
from google.colab import files
pd.DataFrame(bleus2).to_csv('withoutTM.csv')
files.download('withoutTM.csv')

pd.DataFrame(bleus1).to_csv('withoutTM-oto.csv')
files.download('withoutTM-oto.csv')

pd.DataFrame(pred2).to_csv('withoutTM-pred.csv')
files.download('withoutTM-pred.csv')
